...
// const updatePhoto = async (pageNum: number) => {
...
export default function Dashboard() {
  ...
  const deletePhoto = (e: React.MouseEvent<HTMLDivElement>) => {
    startTransition(async () => {
      ...
      await axios
        .delete(`/api/category/${_id}/${group}/${img}`)
        .then(async (res) => {
          await update();
        });
    });
    ...
    <div className="py-[60px] px-[20px] relative">
      ...
      {session ? (
        <div className="bg-white rounded-lg w-full max-w-[1500px] p-[20px] relative max-[576px]:px-[10px] mx-auto">
          <button
            className={`flex items-center gap-1 absolute rounded-50px border border-orange-300 bg-orange-300 py-[2px] px-[10px] top-[-15px] right-[10px] transition duration-300 ${editMode ? "text-fuchsia-800 hover:text-fuchsia-800" : "text-white hover:text-slate-600"}`}
            onClick={handleEdit}
          >
            <div>Edit</div>
            ...
            <div className={`${fixedItem ? "fixed-controlbar" : ""}`}>
              <div className="flex relative text-default items-center py-[10px]">
                <button
                  ...
                  className="w-[30px] h-[30px] shrink-0 max-[840px]:w-[20px]"
                >
                ...
                >
                  {session?.user?.collections?.map((g, index) => (
                    <SwiperSlide style={{ width: "auto" }} key={index}>
                      <div
                        className={`flex items-center rounded-50px border border-teal-400 transition duration-300 hover:bg-teal-500 hover:text-white cursor-pointer py-[2px] ${index === groupIndex ? "bg-teal-500 text-white" : ""} ${editMode ? "px-[10px]" : "px-[20px]"}`}
                        group-id={g.groupId}
                      ...
                      >
                        ...
                        {editMode && (
                          <>
                            <button
                              className="ml-[6px] w-[20px] h-[20px] rounded-full bg-white/70 flex-center border border-slate-400/50 hover:bg-white/100 hover:border-teal-500/50"
                            ...
                            <button
                              className="ml-[3px] w-[20px] h-[20px] rounded-full bg-white/70 flex-center hover:bg-white/100 border border-slate-400/50 hover:border-teal-500/50"
          ...
          <div className="py-[20px] flex flex-wrap">
            {session?.user?.collections ? (
              <>
                {session?.user?.collections.length &&
                ...
                {session?.user?.collections.length &&
                  session?.user?.collections[groupIndex]?.photos.map(
                    (photo: Photo, index: number) => (
                      <div
                        className="w-1/5 p-1 max-[840px]:w-1/3 max-[500px]:w-full group hover:z-30"
                        key={index}
                      >
                        <div
                          className="pb-[100%] relative cursor-pointer overflow-hidden"
                          box-wrap=""
                        >
                          <img
                            src={photo?.imgSrc}
                          ...
                          {editMode && (
                            <div className="absolute top-[15px] right-[15px] transition group-hover:opacity-100">
                              <div
                                className="opacity-100 transition-all w-[30px] h-[30px] flex-center bg-rose-400 rounded-full hover:bg-rose-600"
                                onClick={deletePhoto}
                              >
                                <FontAwesomeIcon
                                  icon={faTrash}
