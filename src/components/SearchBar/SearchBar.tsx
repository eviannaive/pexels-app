...
const SearchBar = ({ event } : { event: ()=>void}) => {
  let {initFetch,inputRef,searchBtnShow,setSearchBtnShow,setInputValue,isPending, startTransition} : any = useSearchContext();
  ...
  return (
    <label className={`bg-white rounded-50px inline-flex items-center py-[6px] px-[8px] text-lg border-0 w-full max-w-[400px] max-[840px]:px-[5px] max-[840px]:py-[4px] ${isPending? 'pointer-events-none' : ''}`}>
