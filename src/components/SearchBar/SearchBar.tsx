...
const SearchBar = ({ event } : { event: ()=>void}) => {
  ...
  return (
    <label className={`bg-white rounded-50px inline-flex items-center py-[6px] px-[8px] text-lg border-0 w-full max-w-[400px] ${loading? 'pointer-events-none' : ''}`}>
      <input type="text" placeholder={`${initFetch.current} ...`} className='bg-white/0 focus:outline-none text-slate-700 font-medium grow px-4' onChange={inputHandler} onKeyDown={keyPress} ref={inputRef}/>
